...
1. **Ingestion of data:** We continuously ingest data of the 240 most popular and traded stocks of the US market into our backend through Kafka. Pathway consumes these Kafka topics through its Kafka connector. We also ingest real-time news related to these companies, as well as their sentiment and relevance score, through an API into the `news_sentiment` Kafka topic.
2. **Technical and Sentiment Analysis:** For each of the stocks present in our list, Pathway calculates the technical metrics *(present in Appendix 1.5)*. The metrics are computed as incremental transformations on Pathway streaming tables for low latency. Pathway's key capabilities—live indexing, incremental updates, materialized state tables with persistence, and low-latency joins and windowing—enable instant signal updates as new data arrives, ensuring sub-second response times that eliminate costly batch solutions used by competitors with Spark or Flink. Pathway performs a real-time, stateful temporal `asof_join` on `symbol` and `ts_ms` directly inside the stream engine, combining stock price indicators and news sentiments in-memory. This completely unifies calculations and sentiments upstream, completely bypassing database polling in the critical execution loop.
3. **Models used:** We use ARMA models to forecast future values and GARCH models to predict volatility for each stock, computed continuously over sliding windows to make instant real-time predictions. To capture non-linearity and handle numerous features, we employ two XGBoost models trained on historical price data and technical metrics across all stocks. The first predicts percentage price change, while the second identifies significant moves (changes > 2%). Our key novelty is combining streaming technical indicators, sentiment analysis via upstream Pathway temporal joins, and completely decoupled, stateless real-time ML inference in a single high-throughput pipeline. All computation pipelines run continuously in the backend, minimizing latency when a user requests advice.
4. **Data Handling:** Once the unified calculations and joined sentiments are compiled upstream, the finished feature vector is pushed through Kafka to ClickHouse's `final_table` in one go. ClickHouse stores the historical data which will be used to retrain ARMA, GARCH and XGBoost on the past data. We also establish a dedicated `clickhouse_monitoring` database instance to capture centralized logs, latency telemetry, and container metrics without impacting the main financial storage performance.
...
We ensure expensive operations like model inference only run after basic computations are validated, reducing cascading failures. For monitoring, we implemented structured logging through Python and Pathway to track ingestion status, latency, and failures. We also deploy an isolated telemetry ClickHouse container to persist end-to-end processing delays, consumer group lags, and microservices resources. Before deployment, we validate everything through historical data simulations, schema validation on Kafka payloads to catch malformed data, and safety checks that convert low-confidence signals to "Hold" recommendations. *(See Appendix 1.4)*
...
2. **Synchronizing data from stock ticks and news stream:** The news data is fetched from the API, but stock ticks are updated dynamically. Polling databases introduces latency bottlenecks and couples failure domains. We solved this by shifting the temporal join upstream into the Pathway stream engine. Pathway performs a real-time, stateful `asof_join` on `symbol` and `ts_ms` across the two event streams. The downstream machine learning inference is completely decoupled and stateless, achieving sub-millisecond execution speeds.
